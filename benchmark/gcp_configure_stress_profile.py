...
# File paths
scylla_servers_path = '../gcp/ansible_install/scylla_servers.yml'
...
def fetch_instances_by_label(custom_name, gcp_project, label):
    client = compute_v1.InstancesClient()
    request = compute_v1.AggregatedListInstancesRequest(project=gcp_project)
    agg_list = client.aggregated_list(request=request)
    instances = []
    for zone, response in agg_list:
        if response.instances:
            for instance in response.instances:
                # Match instances based on labels
                
                if instance.labels.get('project') == custom_name and instance.labels.get('type') == label:
                    for interface in instance.network_interfaces:
                        if interface.access_configs:
                            #print(interface.access_configs[0])
                            instances.append(interface.access_configs[0].nat_i_p)
    return instances


# Fetch nodes addresses from GCP Compute Engine instances
def fetch_nodes_from_gcp(custom_name, gcp_project):
    return fetch_instances_by_label(custom_name, gcp_project, 'scylla')

def fetch_loaders_from_gcp(custom_name, gcp_project):
    return fetch_instances_by_label(custom_name, gcp_project, 'loader')
...
gcp_project_id = variables_cfg['gcp_project_id']
key_pair_name = variables_cfg['key_pair_name']
...
def process_regions(variables):
    ...
    for region, details in variables['regions'].items():
        ...
        nodes = fetch_nodes_from_gcp(variables['cluster_name'], gcp_project=gcp_project_id)
        loaders = fetch_loaders_from_gcp(variables['cluster_name'], gcp_project=gcp_project_id)
...
template_path = '../gcp/ansible_install/benchmark_start_load.yml.tpl'
output_yaml_path = '../gcp/ansible_install/benchmark_start_load.yml'
