...
- name: Run stress
  ...
  tasks:
    ...
    - name: Run YCSB on each CPU
      shell: |
        for i in $(seq 0 $(({{ num_cpus }} - 1))); do
          taskset -c $i sh /home/ubuntu/YCSB/bin/ycsb.sh run scylla -P /home/ubuntu/ycsb-workload.yaml -s -threads {{ stress_options.threads }} -p target=$(({throttle} / {{ num_cpus }})) -p scylla.hosts={{ ip_list }} > {{ remote_path }}{{ output_file }}.$i.data &
        done
        wait
      when: seq_populate is defined
